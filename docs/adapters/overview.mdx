...
When `manualActivityDetection` is enabled, the matching Gemini session config must disable automatic
activity detection. The adapter then sends explicit activity start/end markers using its local voice
detector.
