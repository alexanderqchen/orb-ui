...
The two activity-detection settings are paired: the server configuration disables automatic VAD,
and `manualActivityDetection` makes the adapter send explicit `activityStart` and `activityEnd`
markers from its local speech detector. This is the tested, deterministic voice-turn path. If your
Gemini session uses server-side VAD instead, omit both settings.
