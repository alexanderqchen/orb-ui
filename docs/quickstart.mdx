...
Controlled mode is the universal fallback. Use it when your app already owns the voice session and
can provide a conversation state plus normalized input and output speech envelopes.

```tsx
import { Orb } from 'orb-ui'

export function VoiceAgentStatus({ state, inputVolume, outputVolume }) {
  return <Orb signal={{ state, inputVolume, outputVolume }} theme="circle" size={240} />
}
```
...
Both volume directions should be numbers from `0` to `1`.
