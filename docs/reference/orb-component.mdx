...
| `style`       | `OrbStyle`           | none                   | Root styles and typed `--orb-ui-*` design variables.               |
| `slotProps`   | `OrbSlotProps`       | none                   | Classes, styles, and attributes for stable built-in theme slots.   |
| `components`  | `OrbThemeComponents` | none                   | Replacement radial control icon and connecting indicator.          |
| `renderTheme` | `OrbThemeRenderer`   | none                   | Fully custom artwork with normalized signal and lifecycle props.   |
...
Application-wide visual defaults can be supplied without changing adapter ownership:

```tsx
import { OrbThemeProvider, defineOrbTheme } from 'orb-ui'

const theme = defineOrbTheme({
  name: 'circle',
  preset: 'calm',
  appearance: { colors: { speaking: '#f472b6' } },
})

;<OrbThemeProvider theme={theme} size={260}>
  <Orb adapter={primaryAdapter} />
  <Orb adapter={secondaryAdapter} />
</OrbThemeProvider>
```

For responsive layout, set `--orb-ui-size`; for stable internal styling, use `slotProps`; for
complete artwork ownership, use `renderTheme`. All three retain Orb's normalized signal contract.
See [theme customization](/themes/customization#responsive-sizing-and-css-variables),
[stable slots](/themes/customization#stable-slots), and
[custom renderers](/themes/customization#completely-custom-renderers).
